...
let componentsLists = fs
  .readdirSync(path.join(appDirectory, 'src/components'))
  .filter(item => {
    return fs.statSync(path.join(appDirectory, 'src/components', item)).isDirectory();
  });
...
module.exports = class extends Generator {
  ...
  writing() {
    if (this.props.type === 'routes') {
      ...
      if (isExist) {
        this.log(
          yosay('创建失败，因为 ' + chalk.red(this.props.routesNme) + ' 已经存在!')
        );
      } else {
        let routesTmpl = _.template(this.fs.read(this.templatePath('./Feed/Feed.jsx')));
        this.fs.write(
          this.destinationPath(
            `src/routes/${this.props.routesNme}/${this.props.routesNme}.jsx`
          ),
          routesTmpl({
            routesName: this.props.routesNme
          })
        );
        this.fs.copy(
          this.templatePath('./Feed/Feed.less'),
          this.destinationPath(
            `src/routes/${this.props.routesNme}/${this.props.routesNme}.less`
          )
        );
      }
      ...
      if (isExist) {
        ...
        let routesTmpl = _.template(this.fs.read(this.templatePath('./Feed/Feed.jsx')));
        this.fs.write(
          this.destinationPath(`src/components/${this.props.routesNme}.jsx`),
          routesTmpl({
            routesName: this.props.routesNme
          })
        );
        this.fs.copy(
          this.templatePath('./Feed/Feed.less'),
          this.destinationPath(
            `src/components/${this.props.routesNme}/${this.props.routesNme}.less`
          )
        );
      }
    }
